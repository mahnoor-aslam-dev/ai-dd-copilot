    ...
    <DashboardLayout>
      ...
      {!chatOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-r from-goldbright to-gold shadow-lg shadow-black/40 transition-transform hover:scale-105"
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0A0A0C" strokeWidth="2.2">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
          </svg>
          {messages.length === 0 && (
            <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-flag text-[9px] font-bold text-white">
              ?
            </span>
          )}
        </button>
      ...
      {chatOpen && (
        <div
          className="fixed bottom-6 right-6 z-40 flex w-[380px] max-w-[calc(100vw-3rem)] flex-col rounded-2xl border border-white/[0.1] bg-ink2/95 shadow-2xl shadow-black/60 backdrop-blur-xl"
          style={{ height: 500 }}
        >
          <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5">
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-r from-goldbright to-gold">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#0A0A0C" strokeWidth="2.4">
                  <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                </svg>
              </span>
              <span className="font-display text-[13px] font-bold text-parchment">Ask Anything</span>
            </div>
            <button onClick={() => setChatOpen(false)} className="text-dim2 hover:text-parchment transition-colors">
            ...
            {messages.length === 0 ? (
              ...
              messages.map((m, i) => (
                <div key={i}>
                  {m.role === 'user' ? (
                    <div className="flex justify-end">
                      <div className="max-w-[85%] rounded-2xl bg-gradient-to-r from-goldbright to-gold px-3.5 py-2 text-[12.5px] font-medium text-ink">
                        {m.text}
                      </div>
                    </div>
                  ) : (
                    <div className={`mt-2 rounded-xl border p-3.5 ${m.error ? 'border-flag/30 bg-flag/[0.06]' : 'border-white/[0.06] bg-white/[0.02]'}`}>
                      <p className="whitespace-pre-wrap text-[12.5px] leading-relaxed text-parchment/90">{m.text}</p>
                      {m.sources?.length > 0 && (
                        <div className="mt-2.5 flex flex-wrap gap-1.5 border-t border-white/[0.06] pt-2.5">
                          {m.sources.map((s, si) => (
                            <span key={si} className="rounded-full bg-moss/10 px-2 py-0.5 font-mono text-[9.5px] text-moss">
                              doc #{s.document_id} · {s.relevance_score}
                            </span>
                          ))}
                        </div>
          ...
          <form onSubmit={handleAsk} className="border-t border-white/[0.06] p-3">
            <div className="flex items-center gap-2 rounded-full border border-white/[0.1] bg-white/[0.03] py-1 pl-4 pr-1 transition-colors focus-within:border-goldbright/50">
              <input
                type="text"
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                disabled={readyDocsCount === 0}
                placeholder={readyDocsCount === 0 ? 'Waiting for a document…' : 'Ask anything…'}
                className="w-full bg-transparent text-[12.5px] text-parchment placeholder:text-dim2 outline-none focus:outline-none disabled:cursor-not-allowed"
              />
              <button
                type="submit"
                disabled={!question.trim() || asking || readyDocsCount === 0}
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-goldbright to-gold text-ink transition-transform hover:scale-105 disabled:opacity-30 disabled:hover:scale-100"
              >
