...
export default function Overview() {
  ...
  const [uploadMessage, setUploadMessage] = useState(null); // { type: 'success' | 'error', text, caseId }
  const fileInputRef = useRef(null);
  const targetCaseId = useRef(null);
  ...
  function handleUploadClick() {
    ...
    if (cases.length === 0) {
      // Koi case nahi hai, upload karne se pehle ek banana zaroori hai
      window.location.href = '/cases/new';
      return;
    }
    if (cases.length === 1) {
      targetCaseId.current = cases[0].id;
      fileInputRef.current?.click();
      return;
    }
    // Multiple cases hain — pehle pooch lein kis case mein upload karna hai
    setShowCasePicker(true);
  }

  function pickCaseAndUpload(caseId) {
    targetCaseId.current = caseId;
  ...
  async function handleFileSelected(e) {
    ...
    e.target.value = '';
    if (!file || !targetCaseId.current) return;

    setUploading(true);
    setUploadMessage(null);
    try {
      await uploadDocument(targetCaseId.current, file);
      const caseTitle = cases.find((c) => c.id === targetCaseId.current)?.title || 'the case';
      setUploadMessage({ type: 'success', text: `Uploaded to "${caseTitle}". It's being processed now.`, caseId: targetCaseId.current });
    } catch (err) {
      setUploadMessage({ type: 'error', text: err.response?.data?.error || 'Upload failed. Please try again.' });
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <DashboardLayout>
      {/* Hidden file input, shared by the quick-action button */}
      <input ref={fileInputRef} type="file" accept=".pdf,.docx" onChange={handleFileSelected} className="hidden" />

      <div>
        <p className="reveal r1 font-mono text-[11px] uppercase tracking-[0.25em] text-gold">welcome back</p>
        <h1 className="reveal r2 mt-2 font-display text-5xl font-black text-parchment">Hello, {firstName}!</h1>
        <p className="reveal r3 mt-3 max-w-lg text-[14px] leading-relaxed text-dim">
          {cases.length === 0
            ? 'You have no cases yet. Start your first one to begin uploading documents and asking questions.'
      ...
      {uploadMessage && (
        <div
          className={`reveal mt-5 flex items-center justify-between gap-3 rounded-xl border px-4 py-3 text-[13px] ${
            uploadMessage.type === 'success' ? 'border-moss/30 bg-moss/10 text-moss' : 'border-flag/30 bg-flag/10 text-flag'
          }`}
        >
          <span>{uploadMessage.text}</span>
          {uploadMessage.type === 'success' && (
            <Link to={`/cases/${uploadMessage.caseId}`} className="shrink-0 font-mono text-[11px] uppercase underline underline-offset-4">
              View case →
            </Link>
          )}
        </div>
      ...
      {/* Quick actions */}
      <div className="reveal r4 mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Link
          to="/cases/new"
          className="rounded-xl border border-gold/40 bg-gold/[0.06] p-5 transition-colors hover:bg-gold/[0.1]"
        >
          <p className="font-mono text-[11px] uppercase tracking-wide text-gold">start</p>
          <p className="mt-2 font-display text-xl font-bold text-parchment">Begin a new case</p>
        </Link>

        <button
          onClick={handleUploadClick}
          disabled={uploading}
          className="rounded-xl border border-white/[0.08] bg-ink2/50 p-5 text-left transition-colors hover:bg-ink2/80 disabled:opacity-60"
        >
          <p className="font-mono text-[11px] uppercase tracking-wide text-dim2">
            {uploading ? 'uploading…' : 'quick action'}
          </p>
          <p className="mt-2 font-display text-xl font-bold text-parchment">Upload document</p>
        </button>

        <Link
          to="/cases"
          className="rounded-xl border border-white/[0.08] bg-ink2/50 p-5 transition-colors hover:bg-ink2/80"
        >
          <p className="font-mono text-[11px] uppercase tracking-wide text-dim2">review</p>
          <p className="mt-2 font-display text-xl font-bold text-parchment">All your cases</p>
        </Link>
      </div>

      {/* Case picker modal — sirf tab dikhta hai jab multiple cases hon */}
      {showCasePicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-6" onClick={() => setShowCasePicker(false)}>
          <div
            className="w-full max-w-sm rounded-2xl border border-white/[0.1] bg-ink2 p-5 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <p className="font-display text-lg font-bold text-parchment">Upload to which case?</p>
            <div className="mt-4 space-y-1.5">
              {cases.map((c) => (
                <button
                  key={c.id}
                  onClick={() => pickCaseAndUpload(c.id)}
                  className="block w-full rounded-lg border border-white/[0.06] px-3.5 py-2.5 text-left transition-colors hover:bg-white/[0.04]"
                >
                  <p className="text-[13px] text-parchment">{c.title}</p>
            ...
            <button
              ...
              className="mt-4 w-full rounded-full border border-white/[0.08] py-2 font-mono text-[11px] uppercase text-dim hover:text-parchment transition-colors"
            >
              Cancel
            </button>
          </div>
      ...
      {/* Recent cases */}
      <div className="reveal r5 mt-10">
        <p className="font-mono text-[10.5px] uppercase tracking-[0.2em] text-dim2">recent cases</p>
        {loading ? (
          <p className="mt-4 text-[13px] text-dim">Loading…</p>
        ) : cases.length === 0 ? (
          <div className="mt-4 rounded-xl border border-dashed border-white/[0.1] p-10 text-center">
            <p className="font-display text-xl font-bold text-parchment">Nothing here yet</p>
            <p className="mt-1.5 text-[13px] text-dim">Your first case will appear here once created.</p>
            <Link to="/cases/new" className="mt-4 inline-block rounded-full bg-gradient-to-r from-goldbright to-gold px-5 py-2 font-mono text-[12px] font-semibold text-ink">
              + new case
            </Link>
          </div>
        ) : (
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {cases.slice(0, 6).map((c) => (
              <Link
                key={c.id}
                to={`/cases/${c.id}`}
                className="rounded-xl border border-white/[0.06] bg-ink2/50 p-4 transition-colors hover:bg-ink2/80"
              >
                <p className="font-mono text-[10px] text-dim2">#{String(c.id).padStart(3, '0')}</p>
                <p className="mt-1.5 font-display text-lg font-bold text-parchment">{c.title}</p>
                {c.description && <p className="mt-1 line-clamp-2 text-[12.5px] text-dim">{c.description}</p>}
              </Link>
            ))}
          </div>
        )}
